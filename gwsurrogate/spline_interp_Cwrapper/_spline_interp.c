...
spline_plan *spline_prepare(const double *restrict data_x, long data_size)
{
    ...
    if (SPLINE_UNLIKELY(!mem)) return NULL;
                             ...
                             long num_datasets)
{
    ...
    unsigned char *mem = (unsigned char *)malloc(c_sz + rhs_sz);
    ...
    /* -------------------------------------------------------------------
     * Phase 2: Evaluate spline at each out_x point.
     *
     * Precompute per-x geometry (t, inv_h[idx], h[idx]/6) shared across
     * all datasets, then loop over datasets in the inner loop.
     *
    ...
    for (long j = 0; j < out_size; ++j) {
        const double xj = out_x[j];

        const int idx = hunt(data_x, (int)n, xj, prev_idx);
        ...
        const double t       = xj - data_x[idx];
        const double inv_hi  = inv_h[idx];
        const double hi_inv6 = h[idx] * (1.0 / 6.0);

        /* Inner loop: all datasets share the same idx, t, inv_hi, hi_inv6.
         * Future SIMD can vectorise over d by loading c and y as vectors. */
#if defined(__clang__)
#  pragma clang loop vectorize(enable) interleave(enable)
#elif defined(__GNUC__)
#  pragma GCC ivdep
...
int spline_interp_impl(long data_size, long out_size,
                       ...
                       double *restrict out_y)
{
    spline_plan *plan = spline_prepare(data_x, data_size);
    if (SPLINE_UNLIKELY(!plan)) return SPLINE_ERR_OOM;
...
void spline_interp(const long data_size, const long out_size,
                   double const * const data_x, const double  *  const data_y,
                   double const *  const out_x, double *out_y) {
  spline_interp_impl(data_size, out_size,
                     data_x,data_y,
                     out_x, out_y);
}

void spline_interp_multi(const long data_size, const long out_size,
 ...
 * ====================================================================== */
void spline_interp_multi_complex(const long data_size, const long out_size,
                                 const long num_datasets,
                                 const double *restrict data_x,
                                 const double *const *restrict data_y,
                                 const double *restrict out_x,
...
{
    spline_plan *plan = spline_prepare(data_x, data_size);
    if (SPLINE_UNLIKELY(!plan)) return;
    ...
    /* Allocation: 2*n*num_datasets for c arrays (re+im) + 2*ni for rhs scratch */
    const size_t c_sz   = (size_t)(2 * n) * (size_t)num_datasets * sizeof(double);
    const size_t rhs_sz = (size_t)(2 * ni) * sizeof(double);
    unsigned char *mem = (unsigned char *)malloc(c_sz + rhs_sz);
    if (SPLINE_UNLIKELY(!mem)) { spline_plan_free(plan); return; }
    ...
    /* -------------------------------------------------------------------
     * Phase 2: Evaluate spline at each out_x point, write interleaved output.
     * ------------------------------------------------------------------- */
    int prev_idx = 0;
...
#  pragma GCC ivdep
        ...
        for (long d = 0; d < num_datasets; ++d) {
            ...
            /* Real part */
            const double ci_re  = c_re[idx];
            const double ci1_re = c_re[idx + 1];
            ...
            const double val_re = SPLINE_FMA(t,
                                  SPLINE_FMA(t,
                                  SPLINE_FMA(t, d_re, ci_re * 0.5),
                                  b_re),
                                  y_re_i);

            /* Imaginary part */
            const double ci_im  = c_im[idx];
            const double ci1_im = c_im[idx + 1];
            ...
            const double val_im = SPLINE_FMA(t,
                                  SPLINE_FMA(t,
                                  SPLINE_FMA(t, d_im, ci_im * 0.5),
                                  b_im),
                                  y_im_i);

            /* Write interleaved output */
            out_y[d][j*2]     = val_re;
            out_y[d][j*2 + 1] = val_im;
        }
    }
